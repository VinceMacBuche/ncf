...
def update_technique_metadata(technique_metadata):
  ...
  new_meta = '<AGENT type="dsc"><FILES><FILE name="RUDDER_CONFIGURATION_REPOSITORY/dsc/ncf/50_techniques/'+bundle_name+'.ps1"><INCLUDED>true</INCLUDED></FILE></FILES></AGENT>'
  new_elem = xml.etree.ElementTree.fromstring(new_meta)
  metadata_tree.append(new_elem)

  #file = codecs.open(os.path.realpath(metadata_file), "w", encoding="utf-8")
  prettify(metadata_tree)
  #file.close()
